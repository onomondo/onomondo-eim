...
-export([
    psmo_order_to_euiccPackageSigned/3,
    ...
    case Psmo of
        {[{<<"searchCriteria">>, SearchCriteria}, {<<"tagList">>, TagList}]} ->
            case SearchCriteria of
                {[{<<"isdpAid">>, IsdpAid}]} ->
                    {listProfileInfo, #{
                        searchCriteria => {isdpAid, utils:hex_to_binary(IsdpAid)},
                        tagList => utils:hex_to_binary(TagList)
                    }};
                {[{<<"iccid">>, Iccid}]} ->
                    {listProfileInfo, #{
                        searchCriteria => {iccid, utils:hex_to_binary(Iccid)},
                        tagList => utils:hex_to_binary(TagList)
                    }};
...
eco_to_asn_addEim(Eco) ->
    case Eco of
        {[{<<"eimConfigurationData">>, EimCfgEnc}]} ->
            {ok, EimCfg} = 'SGP32Definitions':decode(
                'EimConfigurationData', utils:hex_to_binary(EimCfgEnc)
            ),
            {addEim, EimCfg};
        _ ->
...
result_to_json_listProfileInfoResult(ListProfileInfoResult) ->
    ...
    case ListProfileInfoResult of
        {profileInfoListOk, ProfileInfoListOk} ->
            ...
            ListProfileInfoResultValue =
                {[{finalResult, successResult}, {profileInfoList, ProfileInfoList}]},
...
result_to_json_getRATResult(GetRATResult) ->
    Op2Json = fun(Op) ->
        List = [
            {mccMnc, utils:binary_to_hex(maps:get(mccMnc, Op))},
...
profileInstallationResult_to_outcome(ProfileInstallationResult) ->
    ProfileInstallationResultData = maps:get(
        profileInstallationResultData, ProfileInstallationResult
    ),
    ...
    case FinalResult of
        ...
        _ ->
            [{[{profileInstallationResult, {[{finalResult, errorResult}]}}]}]
    end.
